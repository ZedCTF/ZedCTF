...
const UserManagement = ({ onBack }: UserManagementProps) => {
  ...
  const filterUsers = () => {
    ...
    if (activeTab === "admins") {
      ...
      // Include both "student" and "user" roles in students
      filtered = filtered.filter(user => user.role === "student" || user.role === "user");
  ...
  const confirmDelete = (user: User) => {
    ...
    if (user.role === "admin") {
      setMessage({ 
        type: 'error', 
        text: 'You cannot delete an administrator account. Please demote to user first.' 
      });
      return;
    }
  ...
  const isDeletableUser = (user: User) => {
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case "student":
      case "user": return <GraduationCap className="w-3 h-3" />;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-xs text-muted-foreground">Students/Users</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-6">
          ...
          <TabsTrigger value="students" className="text-xs">Students/Users</TabsTrigger>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle>
                    ...
                    {activeTab === "students" && "Students/Users"}
                ...
                {filteredUsers.map((user) => (
                  <div key={user.id} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="flex items-center space-x-2 flex-shrink-0">
                      ...
                      {/* Delete button - Only show for deletable users */}
